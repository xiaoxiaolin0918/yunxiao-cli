...
| Work item comment **delete/update** | Personal-token **OAPI** still list+create only (raw DELETE 404). **CLI now wraps** AccessKey RPC: `workitem comments delete` / `update` (same `ALIBABA_CLOUD_ACCESS_KEY_*` as `--include-aliyun-uid`). Use `--content-file` for Windows Chinese. |
...
- **0.16.25** — workitem comments delete|update via AccessKey RPC (OAPI still list/create only)
