...
Packages **上传**、Codeup **blame/cherry-pick**、MR label detach、**工作项评论 delete/update**（OAPI 仍仅 list+create；CLI 已通过 AccessKey RPC 封装 `workitem comments delete|update`）等仍无明确 OAPI / 未封装；**Topic/Risk** 工作项类型需在项目设置 UI 启用（CLI 无法启用）。部分类型未启用**迭代**时请省略 `--sprint`。关联类型可用 `ASSOCIATED`/`DEPEND_ON`（`RELATED`/`PARENT_SUB` 常失败）。`profile doctor` 可对照线上字段/工作流。Codeup tags / protected-branches 已支持；`--content-file` 支持绝对路径。详见 [README.md](README.md) 的 Known gaps。

## 变更摘要

- **0.16.25** — AccessKey RPC 接入 workitem comments delete|update（OAPI 仍仅 list/create）
