...
- **npm**：发版后刷新 `npm/checksums.txt`；**默认不** `npm publish`（除非用户明确要求）。
...
```text
# box
git bundle create /workspace/fix-NN-topic.bundle main..branch
# 拷到本机后：
...
2. 分支命名：`fix/NN-topic` 或 `feature/NN-topic`。
3. **TDD** → 实现 → 必要时 bump 版本 → README 双边 changelog 列表格式必须连续：
   `- **0.16.x** — … (#NN)`（用 em dash，勿空行打断上一条的 `- `）。
4. 开 PR；请 **云效评审** + **文档评审**（Critical / Important / Minor + 是否可发版）。
5. Important 修完再合；两边放行后：`gh pr merge` → `git tag vX.Y.Z` → push tag → 等 `release.yml` → 下载 Release `checksums.txt` 写入 `npm/checksums.txt` → commit push main。
